 ...
 *   last modified        : 07. December 2019
 *   version              : 1.0.2.5
 ...
 */
 /**  
 * @author      Ali Güçlü
 * @copyright   Copyright (c) 2020, Mirarus
 *
 */
...
/**
  ...
  * </pre>
  *
  * @author     Stefan Ali Güçlü
  * @param    integer $clid clientID
  * @param    string  $username username
  * @return     array userInfomation
  */
function clientSetServerQueryLogin2($clid, $username) {
  return $this->getData('boolean', 'clientsetserverquerylogin clid='.$clid.' client_login_name='.$this->escapeText($username));
}
...
public function convertSecondsToArrayTime($seconds) {
  ...
  /**
  * convertSecondsToStrTime2
  * 
  * Converts seconds to a strTime2 (Exm. 5 Gün 1 Saat 23 Dakika 19 Saniye)
  *
  * @author     Ali Güçlü
  * @param    integer $seconds  time in seconds
  * @return     string strTime
  */
  public function convertSecondsToStrTime2($seconds) {
    $conv_time = $this->convertSecondsToArrayTime($seconds);
    if ($conv_time['days']) {
      return $conv_time['days'].' Gün '.$conv_time['hours'].' Saat '.$conv_time['minutes'].' Dakika '.$conv_time['seconds'].' Saniye';
    } else{
      if ($conv_time['hours']) {
        return $conv_time['hours'].' Saat '.$conv_time['minutes'].' Dakika '.$conv_time['seconds'].' Saniye';
      } else{
       if ($conv_time['minutes']) {
        return $conv_time['minutes'].' Dakika '.$conv_time['seconds'].' Saniye';
      } else{
        if ($conv_time['seconds']) {
          return $conv_time['seconds'].' Saniye';
        }
      }
    }
  }
...
/**
  * convertSecondsToStrTime3
  * 
  * Converts seconds to a strTime3 (Exm. 5 Gün 1 Sa. 23 Dak. 19 Sn)
  *
  * @author     Ali Güçlü
  * @param    integer $seconds  time in seconds
  * @return     string strTime
  */
public function convertSecondsToStrTime3($seconds) {
  $conv_time = $this->convertSecondsToArrayTime($seconds);
  if ($conv_time['days']) {
    return $conv_time['days'].' Gün '.$conv_time['hours'].' Sa. '.$conv_time['minutes'].' Dak. '.$conv_time['seconds'].' Sn';
  } else{
    if ($conv_time['hours']) {
      return $conv_time['hours'].' Sa. '.$conv_time['minutes'].' Dak. '.$conv_time['seconds'].' Sn';
    } else{
     if ($conv_time['minutes']) {
      return $conv_time['minutes'].' Dak. '.$conv_time['seconds'].' Sn';
    } else{
      if ($conv_time['seconds']) {
        return $conv_time['seconds'].' Sn';
      }
    }
  }
}
